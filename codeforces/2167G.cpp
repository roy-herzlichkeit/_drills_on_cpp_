...
inline static void solver() {
    ...
    int m = int(comp.size());
    vector<ll> fenwick(m + 2, 0);
    auto fenwick_update = [&](int idx, ll val) {
        for (; idx <= m; idx += idx & -idx) fenwick[idx] = max(fenwick[idx], val);
    };
    auto fenwick_query = [&](int idx) {
        ll best = 0;
        for (; idx > 0; idx -= idx & -idx) best = max(best, fenwick[idx]);
        return best;
    };
    ...
    for (int i = 0; i < n; ++i) {
        int idx = get_idx(a[i]);
        ll best_prev = fenwick_query(idx);
        ll cur = best_prev + cost[i];
        fenwick_update(idx, cur);
        if (cur > best_keep) best_keep = cur;
    }

    cout << (total - best_keep) << '\n';
}
