...
void solver() {
    ...
    ll max_level = total / X;
    sort(arr.begin(), arr.end());
    
    vll crossers, fillers;
    for (int i = 0; i < n; i++)
        if(i >= n - max_level)
            crossers.push_back(arr[i]);
        else 
            fillers.push_back(arr[i]);
    ...
    while (fi < (int) fillers.size() || ci < (int) crossers.size()) {
        ll next = (S / X + 1) * X;
        
        while (fi < (int) fillers.size() && S + fillers[fi] < next) {
            order.push_back(fillers[fi]);
            S += fillers[fi];
            fi++;
        }
        
        if (ci < (int) crossers.size()) {
            order.push_back(crossers[ci]);
            S += crossers[ci];
            ci++;
        }
    }
